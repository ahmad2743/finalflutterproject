...
class ApiClient {
  ...
  getNewsResponse(String value) async {
    ...
    String uri = "${_apiService.newsUrl}${_apiService.query}&${_apiService.limit}&$endpoint&${ApiService.apiKey}";
  ...
  getStatsResponse(StateLocation stateLocation,{String code="",bool yesterday=false}) async {
    ...
    String uri =_apiService.statsUrl+endpoint;
    var url = Uri.parse(uri);
    try{
      var response = await http.get(url);
  ...
  _getStatsEndpoint({required String code,bool? yesterday,required StateLocation location}) {
    if (location == StateLocation.GLOBAL) {
      return "all?yesterday=$yesterday";
    }
